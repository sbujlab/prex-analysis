...
TFile* Open(int runnum, TString filestem = "", Int_t segment=0) {
  ...
  TString lagrange_path = "/lustre19/expphy/volatile/halla/parity/prex-respin2/LagrangeOutput/rootfiles/";
  mymul->AddFriend("lagrall",lagrange_path+Form("prexRespin2_lagrange_eigen_%d.000.root",runnum));
  mymul->AddFriend("regall",lagrange_path+Form("prexRespin2_lagrange_eigen_%d.000.root",runnum));
  mymul->SetMarkerStyle(7);
  mymul->SetAlias("sign","(actual_pattern_polarity-0.5)*2");
  return myfile;
}
