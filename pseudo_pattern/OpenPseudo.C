...
TFile* OpenPseudo(int runnum, TString filestem = "", Int_t segment=0) {
  ...
  TString pseudo_path="/lustre19/expphy/volatile/halla/parity/tao/pseudo_pattern/";
  mymul->AddFriend("psm=mul",pseudo_path+Form("Pseudo240Hz_lagrange_eigen_%d.000.root",runnum));
  mymul->AddFriend("lagrall",pseudo_path+Form("Pseudo240Hz_lagrange_eigen_%d.000.root",runnum));
  mymul->AddFriend("regall",pseudo_path+Form("Pseudo240Hz_lagrange_eigen_%d.000.root",runnum));
  mymul->SetMarkerStyle(7);
  return myfile;
}
