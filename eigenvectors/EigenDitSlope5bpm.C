void EigenDitSlope5bpm(Int_t slug=94){
  ...
  vector<TString> DVlist ={"us_avg","us_dd","usl","usr"};
  Int_t nDV = DVlist.size();
  Int_t nIV = IVlist.size();
  TFile* output = TFile::Open(Form("rootfiles/dit_eigslopes_5bpm_slug%d.root",slug),"RECREATE");
  ...
  TTree *lagr_tree = (TTree*)input->Get("mini_dit");// for 5 BPMs ... dithering is the only chose
