...
void GetAsymmetryByBlockFromCut_quick2(Int_t slug){
  ...
  Int_t mini_id = 0;
  for(int i=0;i<4;i++){
    fTreeArray[i]->Branch("run",&run_number);
    fTreeArray[i]->Branch("mini",&mini_id);
  }
  ...
  vector<STAT> fStat_pos(ndev*4);
  vector<STAT> fStat_neg(ndev*4);
  vector<STAT> fStat_neutral(ndev*4);
  STAT fStat_zero;
  fStat_zero.mean=0.0;
  fStat_zero.error=-1;
  fStat_zero.rms=0.0;
  ...
  while(!feof(prex_runlist)){
	...
	for(int iblk=0;iblk<4;iblk++){
	  ...
	  htemp  = (TH1D*)gDirectory->FindObject("htemp");
	  htemp->SetName(Form("htemp%d",counts++));
	  fStat_pos[4*idev+iblk].mean = htemp->GetMean();
	  fStat_pos[4*idev+iblk].error = htemp->GetMeanError();
	  fStat_pos[4*idev+iblk].rms = htemp->GetRMS();
	  // Neg
	  mul_tree->Draw(device_name+block_format[iblk],
			 "ok_cut && actual_pattern_polarity==0"+mini_cut,"goff");
	  htemp  = (TH1D*)gDirectory->FindObject("htemp");
	  htemp->SetName(Form("htemp%d",counts++));
	  fStat_neg[4*idev+iblk].mean = htemp->GetMean();
	  fStat_neg[4*idev+iblk].error = htemp->GetMeanError();
	  fStat_neg[4*idev+iblk].rms = htemp->GetRMS();
	  // neutral
	  mul_tree->Draw("2*(actual_pattern_polarity-0.5)*"+device_name+block_format[iblk],
			 "ok_cut"+mini_cut,"goff");
	  htemp  = (TH1D*)gDirectory->FindObject("htemp");
	  htemp->SetName(Form("htemp%d",counts++));
	  fStat_neutral[4*idev+iblk].mean = htemp->GetMean();
	  fStat_neutral[4*idev+iblk].error = htemp->GetMeanError();
	  fStat_neutral[4*idev+iblk].rms = htemp->GetRMS();
	}
