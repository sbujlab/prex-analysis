...
void GenSlugList(){
  ...
  while(iter_slug!=fSlugArray.end()){
    ...
    TString new_filename = Form("slug%d_info.list",mySlug);
    FILE* output;
...
void GetProductionRunList(){
 ...
 TString query[]={"SELECT t1.run_number,t3.int_value, t7.text_value,CASE WHEN tflag.text_value is NULL THEN 'NoFlag' ELSE tflag.text_value END AS flag_res, t1.text_value, t2.text_value,t6.int_value ",
		  ...
		  ", `a-rcdb`.conditions as t8 INNER JOIN `a-rcdb`.condition_types c8 on c8.id=t8.condition_type_id AND c8.name='target_type'",
  		   " WHERE t1.run_number=t3.run_number AND t1.run_number=t2.run_number AND t1.run_number=t4.run_number AND t1.run_number=t6.run_number AND t1.run_number=t7.run_number  AND t1.run_number=t8.run_number AND (t8.text_value='48Ca' OR t3.int_value>=4000) ",
		   " ORDER BY t1.run_number ASC"};

 TString listName = "all_production_crex.list";
