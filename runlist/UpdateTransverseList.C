  ...
  // Author : Tao Ye
void GetTransverseRunList();
Int_t ParseSlugNumber(TString);
void GenSlugList();

void UpdateTransverseList(){
  GetTransverseRunList();
  GenSlugList();
}
...
void GenSlugList(){
  ...
  ifstream info_file;
  info_file.open("all_transverse.list");
  ...
  while(iter_slug!=fSlugArray.end()){
    TString new_filename = Form("slug%d_info.list",(*iter_slug).first);
    FILE* output;
...
void GetTransverseRunList(){
 ...
 TString query[]={"SELECT t1.run_number,t3.int_value, t7.text_value,CASE WHEN tflag.text_value is NULL THEN 'NoFlag' ELSE tflag.text_value END AS flag_res, t1.text_value, t2.text_value,t6.int_value ",
		  ...
		  ", `a-rcdb`.conditions as t7 INNER JOIN `a-rcdb`.condition_types c7 on c7.id=t7.condition_type_id AND c7.name='target_type' ",
  		   " WHERE t1.run_number=t3.run_number AND t1.run_number=t2.run_number AND t1.run_number=t4.run_number AND t1.run_number=t6.run_number AND t1.run_number=t7.run_number ",
		   " ORDER BY t1.run_number ASC"};

 TString listName = "all_transverse.list";
  ...
  FILE* simple_list = fopen("./simple_list/slug501.list","w");
  FILE* good_list = fopen("./good_list/slug501.list","w");
