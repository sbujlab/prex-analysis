void ShowPhaseDependent(Int_t run_number=3426,
  ...
  TGraph *gusl_signed = (TGraph*)gPad->FindObject("Graph");
  gusl_signed->SetName("usr signed");
  ...
  TGraph *gusr_signed = (TGraph*)gPad->FindObject("Graph");
  gusr_signed->SetName("usr signed");
  // US DD
  ...
  c4->Divide(2,1);
  c4->cd(1);
  TMultiGraph *mgss = new TMultiGraph();
  mgss->SetTitle("Coil5:Corrected Asymmetry(ppm), unsigned; yield_bmod_ramp; asymmetry(ppm)");
  gusr_signed->SetMarkerColor(kGreen);
  gusl_signed->SetMarkerColor(kMagenta);
  mgss->Add(gusl_signed,"p");
  mgss->Add(gusr_signed,"p");
  mgss->Draw("A");
  TLegend *legss = new TLegend(0.1,0.9,0.3,0.7);
  legss->AddEntry(gusr_signed,"USR","p");
  legss->AddEntry(gusl_signed,"USL","p");
  legss->Draw("same");

  c4->cd(2);
  // TMultiGraph *mgss_position = new TMultiGraph();
  
  mul->Draw("(actual_pattern_polarity-0.5)*2*lagr_asym_usr/ppm:yield_bpm4eX/mm",
	    "ErrorFlag==0x4019000 && yield_bmwobj==5","");
  TGraph *gusr_position_signed = (TGraph*)gPad->FindObject("Graph");
  gusr_position_signed->SetName("usr signed");
  gusr_position_signed->SetMarkerColor(kGreen);
  // mgss_position->Add(gusr_position_signed,"p");

  mul->Draw("(actual_pattern_polarity-0.5)*2*lagr_asym_usl/ppm:yield_bpm4eX/mm",
	    "ErrorFlag==0x4019000 && yield_bmwobj==5","same");
  TGraph *gusl_position_signed = (TGraph*)gPad->FindObject("Graph");
  gusl_position_signed->SetName("usl signed");
  gusl_position_signed->SetMarkerColor(kMagenta);
  // mgss_position->Add(gusl_position_signed,"p");

  // mgss_position->SetTitle("Coil5:Corrected Asymmetry(ppm), unsigned; yield_bpm4eX(mm); asymmetry(ppm)");

  // mgss_position->Draw("A");
  // TLegend *legss_position = new TLegend(0.1,0.9,0.3,0.7);
  // legss_position->AddEntry(gusr_signed,"USR","p");
  // legss_position->AddEntry(gusl_signed,"USL","p");
  // legss_position->Draw("same");
}
