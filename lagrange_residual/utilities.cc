...
map< pair<Int_t,Int_t>, vector<Double_t> > GetRegressionSlope(Int_t slug){
  ...
  TFile *mini_merge = TFile::Open(Form("respin2_treeMergeOutput/MergedLagrange_slug%d.root",slug));
...
map< pair<Int_t,Int_t> , Int_t > LoadCyc2BurstMap(Int_t slug, Int_t run){
  ...
  TFile* input = TFile::Open(Form("rootfiles/slug%d_cycle_burst.root",slug));
