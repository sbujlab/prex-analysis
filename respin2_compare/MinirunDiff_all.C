...
map<TString,TString> fLinks={ {"mini_lagrall","MergedLagrange"},
			      {"mini_regall","MergedLagrange"}};
			     

map<TString,TString > fMaps ={ {"reg_all","mini_regall"},
			       {"lagr_all","mini_lagrall"} };
  
vector< pair<TString,TString> > fSets ={{"lagr_all","reg_all"}};

void MinirunDiff_all(){
