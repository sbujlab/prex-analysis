void GrandDifferences(){
  ...
  while(iter!= fLinks.end()){
    ...
    cout << tag << ":" <<  file_name << endl;
    TString prefix;
    if(tag.Contains("Dithering"))
      prefix="dit";
    else if (tag.Contains("Lagrange"))
      prefix="lagr";
    ...
    slug_tree->Draw(Form("sign*%s_asym_us_dd*1e9 : %s_asym_us_dd.error*1e9 :slug + arm_flag/3.0 ",
    ...
    psslug->SetY1NDC(0.5);
    psslug->SetX1NDC(0.5);
    c1->SaveAs(Form("./png/prex_slug_dd_%s.png",tag.Data()));


    pitt_tree->Draw(Form("%s_asym_us_dd*1e9 : %s_asym_us_dd.error*1e9 :pitt ",
			 prefix.Data(),prefix.Data()),
			 "","goff");
    ...
    pspitt->SetY1NDC(0.5);
    pspitt->SetX1NDC(0.5);
    c1->SaveAs(Form("./png/prex_pitt_dd_%s.png",tag.Data()));
