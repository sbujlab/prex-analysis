void EigenDitSlope(Int_t kSwitch, Int_t run){
  ...
  if(kSwitch==0){  // 5 bpm
    ...
    IVlist = IVlist2;
    filename_tag = "allbpm";
  }else if(kSwitch==2){  // all bpm Lagrange with wrong slopes
    vector< TString > IVlist2 =  {"bpm4aX","bpm4eX","bpm1X",
				  "bpm11X","bpm12X","bpm16X",
				  "bpm4aY","bpm4eY","bpm1Y",
				  "bpm11Y","bpm12Y","bpm16Y"};
    IVlist = IVlist2;
    filename_tag = "allbpm_false";
  }
  ...
  TFile* output = TFile::Open(Form("rootfiles/dit_eigslopes_%s_run%d.root",
				   filename_tag.Data(),run),"RECREATE");
  ...
  if(kSwitch==0){
    ...
    lagr_tree = (TTree*)input->Get("mini_lagr_wrg");
    eig_tree = (TTree*)input->Get("mini_eigall");
  }
