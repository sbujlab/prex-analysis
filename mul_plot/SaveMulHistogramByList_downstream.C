...
void SaveMulHistogramByList_downstream(TString list_name);

void SaveMulHistogramByList_downstream(){
  // TString list_array[]={"50uA","50uA_1arm",
  // 			"70uA_120Hz","70uA_120Hz_1arm",
  // 			"70uA_240Hz","70uA_240Hz_1arm",
  // 			"85uA"};
  TString list_array[]={"70uA_240Hz","70uA_240Hz_1arm",
			"85uA"};

  Int_t npt=  sizeof(list_array)/sizeof(*list_array);
  for(int i=0;i<npt;i++){
    SaveMulHistogramByList_downstream(list_array[i]);
  }
}
void SaveMulHistogramByList_downstream(TString list_name){
  ...
  TFile *output = TFile::Open(Form("mul1d_%s_downstream.root",label.Data()),"RECREATE");
