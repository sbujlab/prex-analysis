void DrawSlopes_updownDD(TString det_name,TString cut_text, Int_t slug_start, Int_t slug_end);
void DrawSlopes_updownDD(){
  // DrawSlopes_updownDD("lr_avg_dd","arm_flag==0 &&kGood==1",3,94);   
  // DrawSlopes_updownDD("lr_dd_dd","arm_flag==0 &&kGood==1",3,94);   
  // DrawSlopes_updownDD("l_dd","arm_flag!=1 &&kGood==1",3,94);   
  // DrawSlopes_updownDD("r_dd","arm_flag!=2 &&kGood==1",3,94);   
  // DrawSlopes_updownDD("ds_avg","arm_flag==0 &&kGood==1",3,94);   
  // DrawSlopes_updownDD("us_avg","arm_flag==0 &&kGood==1",3,94);   
  // DrawSlopes_updownDD("us_dd","arm_flag==0 && kGood==1",3,94);
  // DrawSlopes_updownDD("us_avg","arm_flag==0",1,2);   
  // DrawSlopes_updownDD("us_dd","arm_flag==0",1,2);
  TString cut_text = " && run!=3433 && run<=3876 && !(slug>=6 && slug<=11)";
  // DrawSlopes_updownDD("ds_avg","arm_flag==0 &&kGood==1"+cut_text,5,30);   
  // DrawSlopes_updownDD("us_avg","arm_flag==0 &&kGood==1"+cut_text,5,30);   
  DrawSlopes_updownDD("usl","arm_flag==0 &&kGood==1"+cut_text,5,30);   
  DrawSlopes_updownDD("usr","arm_flag==0 &&kGood==1"+cut_text,5,30);   
...
void DrawSlopes_updownDD(TString det_name,TString cut_text, Int_t slug_start, Int_t slug_end){
  ...
  TString plot_filename = Form("plots/slopes_updownDD_evectors_%s_slug%d-%d.pdf",det_name.Data(),slug_start,slug_end);
  ...
  for(int islug=slug_start;islug<=slug_end;islug++){
    eig_tree->AddFile(Form("./rootfiles/slug%d_updownDD_sorted_eigenvector_allbpm.root",islug));
    reg_tree->AddFile(Form("./rootfiles/slug%d_updownDD_sorted_eigenvector_allbpm.root",islug));
    lagr_tree->AddFile(Form("./rootfiles/slug%d_updownDD_sorted_eigenvector_allbpm.root",islug));
    info_tree->AddFile(Form("./rootfiles/slug%d_updownDD_runinfo.root",islug));
  }
