void merge_sorted_trees(TString suffix = "") {
  ...
  TFile *ofile = TFile::Open(Form("dataRootfiles/rcdb_eigenvectors_sorted%s.root",suffix.Data()),"recreate");
  TChain* mini_eigen_reg_allbpms_sorted = new TChain(Form("mini_eigen_reg_allbpms_sorted%s",suffix.Data()));
  ...
  mini->Add(Form("dataRootfiles/rcdb_eigenvectors%s.root",suffix.Data()));
  TChain* mini_eigen_reg_allbpms = new TChain("mini_eigen_reg_allbpms");
  mini_eigen_reg_allbpms->Add(Form("dataRootfiles/rcdb_eigenvectors%s.root",suffix.Data()));
  TChain* mini_eigen_reg_allbpms_tr = new TChain("mini_eigen_reg_allbpms_tr");
  mini_eigen_reg_allbpms_tr->Add(Form("dataRootfiles/rcdb_eigenvectors%s.root",suffix.Data()));
  TChain* mini_eigen_reg_5bpms = new TChain("mini_eigen_reg_5bpms");
  mini_eigen_reg_5bpms->Add(Form("dataRootfiles/rcdb_eigenvectors%s.root",suffix.Data()));
  ofile->cd();
