...
void GenerateRunInfoTreeBySlug(int slug){
  ...
  TFile* input = TFile::Open(Form("treeMergeOutput/plus_bmw/slug%d.root",slug),"RECREATE");
  TTree* parent_tree = (TTree*)input->Get("mini");
  TFile* output = TFile::Open(Form("treeMergeOutput/plus_bmw/slug%d_runinfo.root",slug),"RECREATE");
