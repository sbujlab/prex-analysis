...
void FlipEigenvectors(int start, int end){
  ...
  TFile* output = TFile::Open(Form("rootfiles/flipped_eigenvector_%s.root",
  ...
  for(int ievt=0;ievt<nEntries;ievt++){
    ...
    vector<Int_t> fSign = CheckSignWithRingAvg( fRingAvg, fEigenVectorRaw);
    vector<Double_t> fFlipped = FlipVectors(fEigenVectorRaw, fSign); 
  ...
  TFile *output_avg = TFile::Open("flipped_matrix.root","RECREATE");
  output_avg->cd();
