...
void GenerateRunInfoTreeByFile(TString filename="./rootfiles/PostpanMerged_all_slugs.root"){ // MergePostpan_v2.C
  map<Int_t , vector<Int_t> > fRunInfo  = LoadRunInfo();  
  TFile* input = TFile::Open(filename);
  TTree* parent_tree = (TTree*)input->Get("mini");
  TFile* output = TFile::Open("./rootfiles/runinfo_all_slugs.root","RECREATE");
  ...
  parent_tree->SetBranchAddress("minirun",&burst_counter);
