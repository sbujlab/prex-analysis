...
void MergeDitTreeByBurst(Int_t slug);
void MergeDitTreeByBurst(TString label);
void MergeDitTreeByBurst();

void MergeDitTreeByBurst(){
  for(int i=0;i<=94;i++){
    MergeDitTreeByBurst(i);
  }
}
void MergeDitTreeByBurst(Int_t slug){
  TString label = Form("slug%d",slug);
  MergeDitTreeByBurst(label);
}
...
void MergeDitTreeByBurst(TString label){
  ...
  TFile *output = TFile::Open("./rootfiles/DitMergedByBurst_"+label+".root","RECREATE");
  TTree *dit_sum_tree = new TTree("dits","Dithering summary");
  TTree *dit_burst_tree = new TTree("burst_dit","Dithering Burst summary");
  ...
  vector<TString> at_list ={"dit_asym_atr1","dit_asym_atl1","dit_asym_atr2","dit_asym_atl2",
			    ...
			    "dit_asym_atl1r2_avg","dit_asym_atl1r2_dd",
			    "dit_asym_atr1l2_avg","dit_asym_atr1l2_dd",
  ...
  for(int i=0;i<ndet;i++){
    dit_burst_tree->Branch(det_list[i],&mini_val[i],"mean/D:err:rms");
    dit_sum_tree->Branch(det_list[i],&run_val[i],"mean/D:err:rms");
  }
  ...
  dit_burst_tree->Branch("unit",&aUnit,"ppm/D:ppb:um:mm:nm");
  Int_t run_id,mini_id;
  dit_sum_tree->Branch("run",&run_id);
  dit_burst_tree->Branch("run",&run_id);
  dit_burst_tree->Branch("mini",&mini_id);
  while(!feof(runlist)){
    Int_t run_number=0;
