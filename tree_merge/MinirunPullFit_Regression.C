void MinirunPullFit_Regression(){
  TFile *input = TFile::Open("./rootfiles/MergedLagrange_all_slugs.root");
  TTree *mini_tree = (TTree*)input->Get("mini_regall");
  ...
  mgall->SetTitle("PREX Regression Corrected Asymmetry(ppb) ; run; (ppb)"); 
  gStyle->SetOptFit(1);
  mgall->Draw("AP");
  mgall->Fit("pol0");
  ...
  mini_tree->Draw(Form( "(spin*reg_asym_us_avg*1e9 - %f)/(reg_asym_us_avg.err*1e9)>>hpull",mean_fit),
		  "arm_flag==0 && kGood==1","goff");
  mini_tree->Draw(Form( "(spin*reg_asym_usr*1e9 - %f)/(reg_asym_usr.err*1e9) >>+hpull",mean_fit),
		  "arm_flag==1 && kGood==1","goff");
  mini_tree->Draw(Form( "(spin*reg_asym_usl*1e9 - %f)/(reg_asym_usl.err*1e9) >>+hpull",mean_fit),
		  "arm_flag==2 && kGood==1","goff");
  hpull->Draw();
  hpull->Fit("gaus");
  c1->SaveAs("prex_regression_minirun_pullfit.pdf");
}
