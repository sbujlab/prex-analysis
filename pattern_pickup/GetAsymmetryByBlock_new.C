...
void GetAsymmetryByBlock_new(Int_t slug);
void GetAsymmetryByBlock_new();
void GetAsymmetryByBlock_new(){
  for(int i=1;i<=94;i++)
    GetAsymmetryByBlock_new(i);
}
void GetAsymmetryByBlock_new(Int_t slug){
  ...
  if(prex_runlist==NULL)
    return;
  TString output_filename = Form("./rootfiles/slug%d_by_block_new.root",slug);
