void bmwPlusRMSBySlug(){


  TFile* reg_input = TFile::Open("regall_prod_only_grand_average_signed_weighted.root");
  TFile* reg_bmw_input = TFile::Open("regall_plus_bmw_grand_average_signed_weighted.root");

  TFile* lagr_input = TFile::Open("lagrall_prod_only_grand_average_signed_weighted.root");
  TFile* lagr_bmw_input = TFile::Open("lagrall_plus_bmw_grand_average_signed_weighted.root");

  TFile* dit_input = TFile::Open("dit_prod_only_grand_average_signed_weighted.root");
  TFile* dit_bmw_input = TFile::Open("dit_plus_bmw_grand_average_signed_weighted.root");
  ...
  lagr_tree->Draw("dit_bmw.Adet.rms*1e6*1.05 : slug","arm_flag!=0","goff");
  ...
  leg->AddEntry(mg_lagr_bmw, " Lagrange : Prod+BMW ");
  leg->AddEntry(mg_reg_bmw, " Regression : Prod+BMW");
  leg->AddEntry(mg_dit_bmw, " Dithering : Prod+BMW");
  ...
  mg->SetTitle();
  leg->Draw("same");
  c1->SaveAs("plus_bmw_rms_by_slug.pdf");
}
